...
# tmux send-keys -t $SESSION:0.3 "sleep 2; source /opt/ros/noetic/setup.bash && python3 ~/gestelt_ws/PX4-Autopilot/src/modules/imu_souvik_ai/fake_imu.py" C-m
tmux send-keys -t $SESSION:0.3 "sleep 5; source /opt/ros/noetic/setup.bash && python3 ~/gestelt_ws/PX4-Autopilot/src/modules/imu_souvik_ai/fake_imu.py _udp_port:=$IMU_BRIDGE_PORT" C-m
